...
export default class Graph extends React.Component {
	render() {
		...
		else if (_.max(this.props.data) > 20) {
			scale = 2
		}
		else {	
			scale = 3
		...
		return (
				<svg viewBox = "0 0 200 300">
					...
					{this.props.dates.map((date, index) => <text x={index*10+15} fontSize="4" y={180/scale + 6} transform={`rotate(90 ${index*10+15}, ${180/scale + 5})`} key={index} info={this.props.info}>{date.replace(/_/g, " ") +" "}</text>)}
				</svg>
			)
	}
}
